...
>
  ...
  <input
      ...
      type="{{ $type ?? 'text' }}"
      @class([
        'py-4 px-6 block w-full sm:text-sm sm:leading-5 outline-none ring ring-1 ring-gray-700 focus:ring-gray-800',
        'rounded-lg' => ! ($roundedFull ?? false),
        'rounded-full' => ($roundedFull ?? true),
        'text-center' => $centered ?? false,
      ])
      class="@error($name ?? (\Illuminate\Support\Str::camel($title))) border-red-300 text-red-900 placeholder-red-300 focus:border-red-300 focus:shadow-outline-red @enderror"
      placeholder="{{ $title }}"
